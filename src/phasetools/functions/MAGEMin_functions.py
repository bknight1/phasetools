...
class MAGEMinGarnetCalculator:
    ...
    def _extract_garnet_elements_from_oxides(self, out, sys_in):
        ...
        # Extract garnet oxide amounts from APFU output.
    ...
    def gt_along_path(self, P, T, data, X, Xoxides, sys_in, fractionate=False, rm_list=None, normalize_start=True):
        ...
        normalize_start : bool, default=True
            If ``True``, ignores the initial garnet volume for fractionation purposes.
            If ``False``, any garnet present at the start of the path is fractionated
            immediately.
        ...
        for i, (P_step, T_step) in enumerate(zip(P, T)):
            ...
            if phase_functions is not None:
                if i == 0 and not normalize_start:
                    # Fractionate the initial volume if we are not normalizing
                    if gt_frac > 0:
                        X = phase_functions.fractionate_phase('g', out, sys_in, frac_amount=gt_frac)
                elif i > 0:
...
def single_point_minimization_with_conversion(P, T, data, X, Xoxides, sys_in="wt", rm_list=None):
    """
    Perform single-point minimization with automatic Python-to-Julia conversion, including optional removal list.
    ...
    data : object
        Initialized MAGEMin data object.
    X : list[float]
