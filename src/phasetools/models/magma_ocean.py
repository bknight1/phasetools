...
class MagmaOcean(MAGEMinBase):
    ...
    def run_fractional_stages(self, 
        ...
        p_max, p_min = p_start, p_end
        
        v_total_mo = self.get_volume_between_radii(self.pressure_to_radius(p_start), self.pressure_to_radius(p_end))
        v_step = vol_step * v_total_mo
        
        r_bottom = self.pressure_to_radius(p_max)
        r_top = self.pressure_to_radius(p_min)
        
        current_liquid_vol_frac = 0.5 # Default start for LMO Stage 1
        
        for stage in range(1, n_stages + 1):
            self.X = jlconvert(jl.Vector[jl.Float64], current_melt_comp)
            
            pressures = np.linspace(p_max, p_min, p_intervals)
            stage_results = {
                "stage": stage,
                "p_base_layer": p_max,
                "p_top_layer": None,
            ...
            for P in pressures:
                T = self.find_temperature_at_vol_frac(P, target_solid_frac)
                stage_results["T"].append(T)
                ...
                stage_results["modes"].append(modes)
                stage_results["densities"].append(densities)
            
            current_melt_comp = melt_sum / p_intervals
            total_solid = sum(layer_modes_sum.values())
            stage_results["layer_modes"] = {ph: val / total_solid for ph, val in layer_modes_sum.items()}
            
            # Geometry updates
            # Sinking minerals increase the base radius of the liquid shell,
            # while floating minerals (e.g. plagioclase) decrease the top radius.
            ...
            new_p_max = self.radius_to_pressure(r_bottom)
            new_p_min = self.radius_to_pressure(r_top)
            
            stage_results["p_top_layer"] = new_p_max
            p_max, p_min = new_p_max, new_p_min
