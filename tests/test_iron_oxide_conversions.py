...
class TestFractionConverters(unittest.TestCase):
    ...
    def test_express_bulk_duplicate_oxide_raises(self):
        """Duplicate oxide names in Xoxides must raise ValueError."""
        from phasetools.utils.bulk_rock import express_bulk_in_feo_o_basis
        with self.assertRaises(ValueError) as ctx:
            express_bulk_in_feo_o_basis([1.0, 2.0], ['SiO2', 'SiO2'], fe3_frac=0.1)
        self.assertIn("Duplicate oxide name", str(ctx.exception))

    def test_express_bulk_length_mismatch_raises(self):
        """Mismatched X and Xoxides lengths must raise ValueError."""
        from phasetools.utils.bulk_rock import express_bulk_in_feo_o_basis
        with self.assertRaises(ValueError) as ctx:
            express_bulk_in_feo_o_basis([1.0], ['SiO2', 'FeO'], fe3_frac=0.1)
        self.assertIn("must have the same length", str(ctx.exception))

    def test_express_bulk_feo_only(self):
        """FeO-only bulk composition is converted to FeO + O basis."""
        from phasetools.utils.bulk_rock import express_bulk_in_feo_o_basis
        X = [50.0, 8.33, 0.05]       # SiO2, FeO, H2O
        ox = ['SiO2', 'FeO', 'H2O']
        X2, ox2 = express_bulk_in_feo_o_basis(X, ox, fe3_frac=0.1)
        self.assertAlmostEqual(X2[1], 8.33)               # FeO preserved as FeOt
        self.assertAlmostEqual(X2[3], 0.1 * 8.33 / 2.0)   # O appended
        self.assertEqual(ox2, ['SiO2', 'FeO', 'H2O', 'O'])

    def test_express_bulk_fe2o3_only(self):
        """Fe2O3-only bulk composition is converted to FeO + O basis."""
        from phasetools.utils.bulk_rock import express_bulk_in_feo_o_basis
        X = [50.0, 4.0, 0.05]        # SiO2, Fe2O3, H2O
        ox = ['SiO2', 'Fe2O3', 'H2O']
        X2, ox2 = express_bulk_in_feo_o_basis(X, ox, fe3_frac=0.5)
        # FeOt = 2 * Fe2O3 = 8.0; FeO appended, Fe2O3 removed, O = 0.5 * 8.0 / 2
        self.assertAlmostEqual(X2[ox2.index('FeO')], 8.0)
        self.assertAlmostEqual(X2[ox2.index('O')], 2.0)
        self.assertEqual(X2[ox2.index('Fe2O3')], 0.0)

    def test_zero_d_array_mol_to_wt_returns_float(self):
        """A 0-d numpy scalar input must return a Python float, not an ndarray."""
        result = mol_fractions_to_wt_fractions(np.float64(0.5), ['FeO'], self.mass_dict)
        self.assertIsInstance(result, float)
        self.assertAlmostEqual(result, 0.5 * self.mass_dict['FeO'])

    def test_zero_d_array_wt_to_mol_returns_float(self):
        """A 0-d numpy scalar input must return a Python float, not an ndarray."""
        result = wt_fractions_to_mol_fractions(np.float64(0.05), ['FeO'], self.mass_dict)
        self.assertIsInstance(result, float)
        self.assertAlmostEqual(result, 0.05 / self.mass_dict['FeO'])
